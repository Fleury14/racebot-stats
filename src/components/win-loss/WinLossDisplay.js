...
const WinLossDisplay = (props) => {
  const { playerData, sendRivalInfo, leagueMode } = props;
          ...
          {playerData.opponents.map(player => {
            return (
              <Col md="3" sm="6" key={player.id} className="win-loss-box" onClick={() => { sendRivalInfo(playerData.id, player.id)}}>
                ...
                <h2 className="text-center">{leagueMode ? player.leagueWins : player.wins}-{leagueMode ? player.leagueLosses : player.losses}</h2>
              </Col>
            );
          })}
        </Row>
